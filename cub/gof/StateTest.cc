...
namespace {
  ...
  struct Robot {
    ...
    Robot(Audio& audio)
        : audio(audio), state(nullptr), leftSingTimes(0) {
      __GOTO_STATE(Closed);
    }
...
FIXTURE(GofStateTest) {
  ...
  void gotoExhaustedInWaitChargedState() {
    ...
    FOREACH(i, (int)Robot::max_sing_times_in_one_charge) {
      robot.play();
    }
  }

  uint8_t getSingTimes() const {
    return audio.singTimes;
  }

  uint8_t getBibiTimes() const {
...
protected:
  struct MockAudio: Audio {
    MockAudio()
        : isInMute(true), singTimes(0), bibiTimes(0) {
    }
