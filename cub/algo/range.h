...
#define RANG(c) \
  std::begin(std::forward<Container>(c)), std::end(std::forward<Container>(c))
...
-> decltype(std::begin(std::forward<Container>(c))) {
  return find_if(std::forward<Container>(c), [&t] (const T& e) {
    return e == t;
  });
}
...
namespace details {
  ...
  inline OutputIterator do_filter(InputIterator first, InputIterator last,
...
inline OutputIterator filter(Container&& c, OutputIterator result,
    Predicate pred) {
